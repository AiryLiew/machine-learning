...
from data_sampling import My_Dataset
class Net(torch.nn.Module):
    def __init__(self):
        super().__init__()
        #序列构造器内的程序会自上而下的运行
        self.fc1=torch.nn.Sequential(
        torch.nn.Linear(3*28*28,256),
...
if __name__ == '__main__':
    ...
    #下载数据
    train_data=My_Dataset(r"E:\pycharmprojects\dataset\MNIST\train")
    test_data=My_Dataset(r"E:\pycharmprojects\dataset\MNIST\test")
    ...
    #定义损失函数
    #一般来收，交叉熵损失函数只会被用来做分类损失，均方差损失既可以用来做分类，也可以用来做回归，但是做回归的情况更多
    # loss_fn=torch.nn.CrossEntropyLoss()#多分类（类别大于二）交叉熵损失函数，自动会对标签做one-hot，对输出做softmax
    # loss_fn=torch.nn.BCELoss()#二分类交叉熵损失函数,不会对标签自动做one-hot，需要对输出层做sigmoid激活
    loss_fn=torch.nn.BCEWithLogitsLoss()#二分类损失函数,不会对标签自动做one-hot，和BCELoss相比，自动会对输出层做sigmoid激活
    # loss_fn=torch.nn.MSELoss()#均方差损失函数，需要对标签做one-hot，对输出层没有要求
    #定义优化器
